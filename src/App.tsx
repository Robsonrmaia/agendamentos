...
function taskRelevantDate(task: Task) {
  return task.dueAt ?? task.plannedFor;
}

function scopeMatches(task: Task, scope: Scope, now: Date) {
  if (scope === 'all') return true;
  const value = taskRelevantDate(task);
  if (!value) return false;
  const date = new Date(value);
  if (scope === 'today') {
    if (task.status !== 'done' && date.getTime() < now.getTime()) return true;
    return sameLocalDay(date, now);
  }
  const start = startOfLocalWeek(now);
  const end = new Date(start);
  end.setDate(end.getDate() + 7);
  if (task.status !== 'done' && date < start) return true;
...
function effortLabel(minutes: number | null | undefined) {
...
function isOverdue(task: Task, now = new Date()) {
  return Boolean(task.dueAt && task.status !== 'done' && new Date(task.dueAt).getTime() < now.getTime() && !sameLocalDay(new Date(task.dueAt), now));
}
...
function TaskCard({
  ...
  return (
    <article
      ref={setNodeRef}
      style={style}
      className={`task-card difficulty-${task.difficulty} ${isDragging ? 'dragging' : ''} ${task.status === 'done' ? 'is-done' : ''}`}
      onClick={() => onEdit(task)}
    >
      <div className="card-topline">
        <button
          className={`owner-dot owner-${task.owner}`}
          aria-label={`Trocar responsável de ${task.title}`}
          title={`Responsável: ${task.owner === 'robson' ? 'Robson' : 'Gisele'}`}
          onClick={(event) => { event.stopPropagation(); onOwner(task); }}
        >
          {task.owner === 'robson' ? 'R' : 'G'}
        </button>
      ...
      <div className="card-meta">
        {due && <span className={isOverdue(task) ? 'overdue' : ''}>{isOverdue(task) ? '⚠ ' : '◷ '}{due}</span>}
        {effortLabel(task.estimatedMinutes) && <span>⌁ {effortLabel(task.estimatedMinutes)}</span>}
      </div>
      {task.status === 'waiting' && (
        <div className="dependency-note">↳ {task.dependencyNote || 'Aguardando uma dependência'}</div>
      )}
      <div className="card-actions" onClick={(event) => event.stopPropagation()}>
        {task.status !== 'in_progress' && task.status !== 'done' && (
          <button aria-label="Iniciar agora" title="Iniciar agora" onClick={() => onStatus(task, 'in_progress')}>▶</button>
        )}
        {task.status !== 'waiting' && task.status !== 'done' && (
          <button aria-label="Aguardar" title="Aguardar / Dependência" onClick={() => onStatus(task, 'waiting')}>⌛</button>
        )}
        {task.status !== 'done' && (
          <button aria-label="Finalizar" title="Finalizar" onClick={() => onStatus(task, 'done')}>✓</button>
        )}
        {task.status === 'done' && (
          <button aria-label="Reabrir" title="Reabrir" onClick={() => onStatus(task, 'planned')}>↶</button>
        )}
  ...
  return (
    <section ref={setNodeRef} className={`board-column status-${status} ${isOver ? 'column-over' : ''}`}>
      <div className="column-header">
        <div className="column-title"><span>{statusIcon[status]}</span><h2>{statusLabels[status]}</h2></div>
...
function QuickCapture({
  ...
  function interpret() {
    ...
    try {
      ...
      setPreview({
        title: text.trim(), owner: currentUser, status: 'idea', difficulty: 'medium', sourceText: text,
        confidence: { title: 0.4, owner: 0.5, difficulty: 0, estimatedMinutes: 0, date: 0 },
      });
    }
  }
    ...
    <div className="capture-shell">
      <div className="capture-input-row">
        ...
        <input
          ...
          placeholder="O que você quer fazer? Ex.: terminar OLX até quinta, difícil, 4 horas"
        />
        {speechSupported && (
          <button className={`mic-button ${listening ? 'listening' : ''}`} aria-label="Falar tarefa" onClick={startVoice}>{listening ? '●' : '◉'}</button>
        )}
      ...
      {preview && (
        <div className="capture-preview">
          <div className="preview-main">
            <span className="preview-label">Entendi assim</span>
            <strong>{preview.title}</strong>
          </div>
          <label>
            <span>Responsável</span>
            <select value={preview.owner} onChange={(event) => setPreview({ ...preview, owner: event.target.value as TaskOwner })}>
          ...
          <label>
            ...
            <select value={preview.difficulty} onChange={(event) => setPreview({ ...preview, difficulty: event.target.value as TaskDifficulty })}>
          ...
          <label>
            <span>Prazo</span>
            <input type="date" value={dateInput(preview.dueAt)} onChange={(event) => {
              const dueAt = dateFromInput(event.target.value);
              setPreview({ ...preview, dueAt, status: dueAt ? 'planned' : 'idea' });
            }} />
          </label>
          <label>
            <span>Tempo</span>
            <div className="effort-control">
              <input type="number" min="0" step="15" value={preview.estimatedMinutes ?? ''} onChange={(event) => setPreview({ ...preview, estimatedMinutes: event.target.value ? Number(event.target.value) : undefined })} />
              <b>{effortLabel(preview.estimatedMinutes) || '—'}</b>
            </div>
          </label>
    ...
    <div className="drawer-backdrop" onMouseDown={onClose}>
      <aside className="task-drawer" onMouseDown={(event) => event.stopPropagation()} aria-label="Editar tarefa">
        <div className="drawer-header">
          <div><span>Detalhes da tarefa</span><h2>Editar</h2></div>
  ...
  return (
    <main className="login-page">
      <form className="login-card" onSubmit={login}>
        ...
        <span className="eyebrow">PAINEL COMPARTILHADO</span>
        <h1>Agendamentos</h1>
        <p>Entre para acessar o mesmo painel de Robson e Gisele.</p>
        <label><span>E-mail</span><input type="email" value={email} onChange={(event) => setEmail(event.target.value)} required /></label>
        <label><span>Senha</span><input type="password" value={password} onChange={(event) => setPassword(event.target.value)} required /></label>
...
export default function App() {
  ...
  const summary = useMemo(() => {
    ...
    return {
      today: open.filter((task) => taskRelevantDate(task) && (sameLocalDay(new Date(taskRelevantDate(task)!), now) || new Date(taskRelevantDate(task)!) < now)).length,
      week: open.filter((task) => taskRelevantDate(task) && new Date(taskRelevantDate(task)!) < weekEnd).length,
  ...
  async function persistOne(nextTask: Task, previousTasks: Task[]) {
    setTasks((current) => current.map((task) => task.id === nextTask.id ? nextTask : task));
    if (!supabase || !hasSupabaseConfig) return true;
    try {
      setSyncState('Salvando…');
      await saveSupabaseTask(nextTask);
      setSyncState('Sincronizado');
      return true;
    } catch {
      setTasks(previousTasks);
      setSyncState('Reconectando…');
      setNotice('Não foi possível salvar a alteração. Ela foi desfeita.');
      return false;
    }
  }
  ...
  async function createFromPreview(preview: ParsedTaskDraft) {
    const task = createTask({
      ...
      plannedFor: preview.plannedFor ?? preview.dueAt ?? null,
      dueAt: preview.dueAt ?? null,
      dependencyNote: preview.dependencyNote ?? null,
  ...
  async function moveTask(task: Task, status: TaskStatus) {
    ...
    const nextTask = { ...task, ...transition, dependencyNote, sortOrder: Date.now(), updatedAt: new Date().toISOString() };
    await persistOne(nextTask, tasks);
  }

  async function toggleOwner(task: Task) {
    const nextTask = { ...task, owner: task.owner === 'robson' ? 'gisele' as const : 'robson' as const, updatedAt: new Date().toISOString() };
    await persistOne(nextTask, tasks);
  }

  async function saveEdited(task: Task) {
    await persistOne(task, tasks);
    setEditingId(null);
  ...
  return (
    <div className="app-shell">
      <header className="topbar">
        <div className="brand">
          <div className="brand-mark">A</div>
          <div><span className="eyebrow">CONTROLE RÁPIDO</span><h1>Agendamentos</h1></div>
        </div>
        <div className="top-actions">
          <span className={`sync-pill ${syncState === 'Sincronizado' ? 'online' : ''}`}><i />{syncState}</span>
          {!hasSupabaseConfig && (
            <button className="persona-button" onClick={() => setCurrentUser((user) => user === 'robson' ? 'gisele' : 'robson')}>
              Usando como <b>{currentUser === 'robson' ? 'Robson' : 'Gisele'}</b>
            </button>
          )}
      ...
      <main className="workspace">
        <section className="hero-row">
          <div>
            <span className="eyebrow">VISÃO DO TRABALHO</span>
            <h2>O que precisa andar agora?</h2>
            <p>Fale, digite, arraste. O painel organiza o resto sem virar mais um sistema complicado.</p>
          </div>
          <div className="scope-tabs" aria-label="Período">
            {([['today', 'Hoje'], ['week', 'Esta semana'], ['all', 'Tudo']] as const).map(([value, label]) => (
        ...
        <section className="filters-row">
          <div className="owner-filter" aria-label="Responsável">
            {([['all', 'Todos'], ['robson', 'Robson'], ['gisele', 'Gisele']] as const).map(([value, label]) => (
        ...
        <DndContext sensors={sensors} onDragEnd={handleDragEnd}>
          <section className="board" aria-label="Pipeline de tarefas">
            {statusOrder.map((status) => (
              <TaskColumn
                key={status}
                status={status}
                tasks={visibleTasks.filter((task) => task.status === status)}
                onStatus={(task, nextStatus) => void moveTask(task, nextStatus)}
                onOwner={(task) => void toggleOwner(task)}
